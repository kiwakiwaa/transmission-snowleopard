...
- (void)awakeFromNib
{
    ...
    self.fTimer = TRScheduledTimerWithTimeInterval(kUpdateUISeconds, YES, ^(NSTimer* _Nonnull) {
        [weakSelf updateUI];
    });
...
- (void)openPasteboard
{
    ...
    for (NSString* itemString in arrayOfStrings)
    {
        ...
        NSArray* magnetMatches = [magnetDetector matchesInString:itemString options:0 range:fullRange];
        ...
        {
            BOOL overlapsMagnet = NO;
            for (NSTextCheckingResult* magnetResult in magnetMatches)
            {
                if (NSIntersectionRange(result.range, magnetResult.range).length > 0)
                {
                    overlapsMagnet = YES;
                    break;
                }
...
- (void)VDKQueue:(VDKQueue*)queue receivedNotification:(NSString*)notification forPath:(NSString*)fpath
{
    ...
    __weak __auto_type weakSelf = self;
    self.fAutoImportTimer = TRScheduledTimerWithTimeInterval(10.0, NO, ^(NSTimer* _Nonnull) {
...
- (void)updateMainWindow
{
    if (self.fStatusBar == nil)
    {
        ...
        TRTitlebarAccessoryUseFixedViewSize(self.fStatusBar);
    ...
    if (self.fFilterBar == nil)
    {
        ...
        TRTitlebarAccessoryUseFixedViewSize(self.fFilterBar);
